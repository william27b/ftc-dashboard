...
export default class Integration {
  ...
  beginIntegrationNowMs = Number.NaN; // in telemetry time
  ...
  reset() {
    ...
    this.beginIntegrationNowMs = Number.NaN; // in telemetry time
  ...
  add(time: number, samples: Sample[][]) {
    ...
    if (isNaN(this.beginIntegrationNowMs) && samples.length > 0) {
      const maxT = samples[samples.length - 1].reduce(
        (acc, { name, value }) => (name === 'time' ? value : acc),
        Number.NaN,
      );
      this.beginIntegrationNowMs = maxT - 250; // introduce lag to allow for transmission time
  ...
  render(time: number) {
    ...
    if (isNaN(this.beginIntegrationNowMs)) return false;

    const IntegrationNowMs = this.beginIntegrationNowMs + (time - this.beginRenderTimeMs);

    // prune old samples
    for (const k of Object.keys(this.data)) {
      const { ts, vs } = this.data[k];
      while (ts.length > 0 && ts[0] + o.windowMs + 250 < IntegrationNowMs) {
        ts.shift();
        vs.shift();
      }
    }
    ...
    this.renderIntegration(0, keyHeight, width, height - keyHeight, IntegrationNowMs);
  ...
  renderIntegration(
    ...
    width: number,
    height: number,
    IntegrationNowMs: number,
  ) {
    const o = this.options;

    const IntegrationHeight = height - 2 * o.padding;
    ...
    const IntegrationWidth = width - axisWidth - 3 * o.padding;
  ...
  renderIntegrationLines(
    ...
    Object.keys(this.data).forEach((k, i) => {
      ...
      fineMoveTo(
        this.ctx,
        this.scaling,
        scale(ts[0] - IntegrationNowMs + o.windowMs, 0, o.windowMs, 0, width),
      ...
      for (let j = 1; j < ts.length; j++) {
        fineLineTo(
          this.ctx,
          this.scaling,
          scale(ts[j] - IntegrationNowMs + o.windowMs, 0, o.windowMs, 0, width),
